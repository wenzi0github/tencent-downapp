/*!
 * AppDownload v0.0.1
 * last update: 2018-6-30, author: skeetershi
...
(function (global, factory) {
   ...
   * @param {*} sid 
   */
  var getScript = function getScript(url, callback, sid) {
  ...
  applyMethods.prototype.trigger = function (type) {
      for (var _len = arguments.length, params = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
          params[_key - 1] = arguments[_key];
      }

      var method = this.methods[type];

      method.length && method.forEach(function (item) {
  ...
  */
  ...
  var AppDownload = function () {
      function AppDownload(params) {
          classCallCheck(this, AppDownload);
      ...
      createClass(AppDownload, [{
          key: 'on',
          value: function on(fn, callback) {
          ...
          key: 'checkAppIsInstalled',
          value: function checkAppIsInstalled() {
              ...
              var checkInVideo = function checkInVideo() {
                  // 腾讯视频
                  var ssVideo = function ssVideo() {
                      var pkgName = {
                  ...
                  if ((typeof TenvideoJSBridge === 'undefined' ? 'undefined' : _typeof(TenvideoJSBridge)) === 'object') {
                      ssVideo();
                  } else {
                  ...
                  checkInQQ = function checkInQQ() {
                  // qq
                  var ssQQ = function ssQQ() {
                  ...
                  checkInWxx = function checkInWxx() {
                  var ssWx = function ssWx() {
                  ...
                  checkInNews = function checkInNews() {
                  var ssNews = function ssNews() {
                      if (os.android) {
                  ...
                  if ((typeof TencentNews === 'undefined' ? 'undefined' : _typeof(TencentNews)) == 'object') {
                      ssNews();
                  } else {
                  ...
                  checkInMusic = function checkInMusic() {
                  var ssMusic = function ssMusic() {
                      var install_params = {
                  ...
                  if ((typeof WebViewJavascriptBridge === 'undefined' ? 'undefined' : _typeof(WebViewJavascriptBridge)) == 'object') {
                      ssMusic();
                  } else {
          ...
          key: 'openArticleInNews',


          // 在新闻客户端内打开文章
          value: function openArticleInNews() {
          ...
          key: 'checkOpenInNews',
          value: function checkOpenInNews() {
              var self = this;
              ...
              if (os.android) {
                  ...
                  TencentNews.checkCanOpenNativeUrl(self.openUrl, function (url, result, userInfo) {
                      if (result) {
                          // 打开
                          TencentNews.openNativeUrl(self.openUrl, function (ourl, oresult, ouserInfo) {}, null);
                      } else {
                          // 下载
                          TencentNews.downloadAppInNative(self.appleStoreId, self.downloadUrl, function (durl, dresult, duserInfo) {}, null);
                      }
                  }, null);
              }
          }
          ...
          key: 'checkInstallInMusic',
          value: function checkInstallInMusic() {
              var self = this,
                  install_params = {
                  android: [self.packageName],
                  ios: [self.openUrl]
          ...
          key: 'openAppInMusic',
          value: function openAppInMusic() {
              var self = this;

              if (os.android) {
                  this.openApp();
              } else {
                  var ssOpen = function ssOpen() {
                      var open_params = {
                          url: self.openUrl,
                          target: 'app',
                          type: 'default'
          ...
          key: 'downloadInMusic',
          value: function downloadInMusic() {
              // 未安装
              var self = this,
                  down_params = {
          ...
          key: 'TenvideoReady',
          value: function TenvideoReady(status) {
              // 监听视频js全局对象是否存在
              var self = this;
              if ((typeof TenvideoJSBridge === 'undefined' ? 'undefined' : _typeof(TenvideoJSBridge)) === 'object') {
          ...
          key: 'handleTenvideo',
          value: function handleTenvideo(stat) {
              var self = this;
              var pkgName = {
                  "pkgName": self.packageName,
                  "pkgUrl": self.openUrl
                  ...
                  */
                  ...
                  if (ret.state == 13) ; else if (ret.state == 11) ; else if (ret.state == 10) {
                      // self.installSuccess();
                      self.method.trigger('installSuccess');
                  }
              });
          ...
          key: 'handleWx',
          value: function handleWx(status) {
              var self = this;
              if (!window.WeixinJSBridge) {
                  document.addEventListener('WeixinJSBridgeReady', function () {
          ...
          key: 'checkInWx',
          value: function checkInWx(status) {
              var self = this;
          ...
          // 使用微信提供的api下载
          value: function downloadAppInWx() {
              var self = this;
              ...
              if (os.android) {
                  ...
                  WeixinJSBridge.invoke("addDownloadTask", {
                      ...
                      if (wx_version >= wx_d_version) {
                          if (res.err_msg == 'addDownloadTask:cancel') ; else if (res.err_msg == 'addDownloadTask:ok') ;
                      } else {
          ...
          key: 'cancelDownloadInWx',


          // 取消下载任务
          value: function cancelDownloadInWx() {
              var self = this;
          ...
          key: 'installDownload',
          value: function installDownload() {
              var self = this;
